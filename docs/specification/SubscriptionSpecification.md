...
| **Enterprise** | Rp 499.000 | 0 | 999 | 999 |

> Harga satuan/bulan (IDR). Nilai max users/branches dari `PlanSeeder` (Enterprise = 999).
...
| cash_register | ❌ none | ✅ full | ✅ full | ✅ full |
| master_data | ❌ none | ✅ full | ✅ full | ✅ full |
...
\* cash_register/master_data pada Trial: **none** (tidak disertakan di `PlanSeeder` → `FeatureEngine` mengembalikan `none`).
...
|---|---|---|---|---|
| Max users | 1 | 3 | 10 | 999 |
| Max branches | 1 | 1 | 5 | 999 |
...
**Terkonfirmasi:** nama plan (trial/basic/pro/enterprise), harga, hari trial, fitur utama & batas users/branches dari `PlanSeeder` (Trial 1/1, Basic 3/1, Pro 10/5, Enterprise 999/999; Trial `cash_register`/`master_data` = `none`).

**Perlu Verifikasi:**
- Enforce limit (user ke-4 saat Basic) di sisi server (saat ini belum di-enforce).
