...
class TenantManagementController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $tenantId = 'tenant_' . \Illuminate\Support\Str::random(10);
        $subdomain = \Illuminate\Support\Str::slug($validated['tenant_name']) . '-' . \Illuminate\Support\Str::random(4);
    ...
    public function destroy($tenantId)
    {
        ...
        if (!$tenant) return redirect()->to('/admin/tenants')->with('error', 'Tenant tidak ditemukan.');
        if ($tenant->is_active) return back()->with('error', '❌ Tenant aktif tidak bisa dihapus langsung.');

        $name = $tenant->tenant_name;
        try {
            try { $tenant->database()->manager()->deleteDatabase($tenant); } catch (\Exception $e) { SystemLog::warning("Could not delete tenant database: " . $e->getMessage()); }
    ...
    public function syncAllStats()
    {
        ...
        foreach (Tenant::where("is_active", true)->get() as $t) {
            try { TenantStat::syncStats($t); $count++; } catch (\Exception $e) { SystemLog::error("Failed to sync tenant: " . $e->getMessage()); }
        }
    ...
    public function updateDomain(Request $request, Tenant $tenant)
    {
        ...
        $tenant->domains()->delete();
        \Stancl\Tenancy\Database\Models\Domain::create(["tenant_id" => $tenant->id, "domain" => $validated["domain"]]);
    ...
    public function changePlan(Request $request, Tenant $tenant)
    {
        ...
        SystemLog::info("Plan changed for {$tenant->tenant_name}: {$newPlan->name}");
        return back()->with('success', "✅ Plan {$tenant->tenant_name} diubah ke {$newPlan->name}.");
    }
    ...
    public function loginAs(Tenant $tenant)
    {
        try {
            tenancy()->initialize($tenant);
            $ownerUser = \App\Models\Tenant\User::where('role', 'owner')->first() ?? \App\Models\Tenant\User::first();
            if (!$ownerUser) { tenancy()->end(); return back()->with('error', 'Tidak ada user di tenant ini.'); }
            auth()->login($ownerUser);
            session()->put('tenant_id', $tenant->id);
            tenancy()->end();
            $baseDomain = env('CENTRAL_DOMAIN', 'serviceku.my.id');
            return redirect('http://' . ($tenant->domains->first()?->domain ?? $tenant->slug . '.' . $baseDomain) . '/dashboard');
        } catch (\Exception $e) {
    ...
    public function resetPassword(Tenant $tenant)
    {
        try {
            tenancy()->initialize($tenant);
            $owner = \App\Models\Tenant\User::where('role', 'owner')->first() ?? \App\Models\Tenant\User::first();
            if (!$owner) { tenancy()->end(); return back()->with('error', 'Tidak ada user di tenant ini.'); }
