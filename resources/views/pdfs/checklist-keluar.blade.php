...
<head>
    <meta charset="utf-8">
    <title>Checklist Keluar #{{ $service->id }}</title>
    <style>
...
<body>
    ...
    <div class="title-section">
        <h2>LAPORAN CHECKLIST KELUAR (QC)</h2>
    </div>
    ...
    @php 
        $checklist = $service->checklists->where('type', 'keluar')->first(); 
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="text-center {{ in_array($item->item_name, $checklist->checked_items ?? []) ? 'checked' : 'unchecked' }}">
                    {{ in_array($item->item_name, $checklist->checked_items ?? []) ? '✓ Baik' : '✗ Perlu Perhatian' }}
                </td>
                <td></td>
            </tr>
            @endforeach
    ...
    @else
    <p style="color:#999; text-align:center; padding:20px;">Belum ada checklist keluar.</p>
    @endif
    ...
    <div class="signature">
        <table>
            <tr>
                <td>
                    <p>Teknisi/CS</p>
                    <div class="line">{{ $service->technician?->name ?? '______________' }}</div>
                </td>
                <td>
                    <p>Pelanggan (Saat Pengambilan)</p>
