...
const statusColors = { connected: 'sk-bg-success-soft sk-text-success dark:bg-green-900 dark:text-green-200', disconnected: 'sk-bg-hover sk-text-secondary dark:bg-gray-800 dark:sk-text-muted', error: 'sk-bg-danger-soft sk-text-danger dark:bg-red-900 dark:text-red-200' }
  ...
  <div class="space-y-6">
    ...
    <div class="flex flex-wrap gap-2">
      <button v-for="cat in categories" :key="cat.key" @click="selectedCategory = selectedCategory === cat.key ? null : cat.key"
        ...
        :class="selectedCategory === cat.key ? 'bg-primary-600 text-white shadow' : 'sk-bg-card dark:bg-gray-800 border sk-border dark:border-gray-700 hover:border-primary-300 dark:hover:border-primary-600 sk-text-primary dark:text-gray-300'">
        {{ cat.label }}
      </button>
    </div>
    ...
    <div v-if="selectedCategory" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      <KCard v-for="prov in selectedProviders" :key="prov.key" class="relative">
        ...
        <!-- Actions -->
        <div class="flex gap-2 pt-2 border-t sk-border-light dark:border-gray-700">
