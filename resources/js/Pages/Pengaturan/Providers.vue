...
const statusColors = { connected: 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200', disconnected: 'bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-400', error: 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200' }
  ...
  <div class="space-y-6">
    ...
    <div class="flex flex-wrap gap-2">
      <button v-for="cat in categories" :key="cat.key" @click="selectedCategory = selectedCategory === cat.key ? null : cat.key"
        ...
        :class="selectedCategory === cat.key ? 'bg-primary-600 text-white shadow' : 'bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 hover:border-primary-300 dark:hover:border-primary-600 text-gray-700 dark:text-gray-300'">
        {{ cat.label }}
      </button>
    </div>
    ...
    <div v-if="selectedCategory" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      <KCard v-for="prov in selectedProviders" :key="prov.key" class="relative">
        <div class="flex items-start justify-between mb-3">
          <div>
            <h3 class="font-semibold text-gray-900 dark:text-white">{{ prov.label }}</h3>
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{{ prov.description || 'Tidak ada deskripsi' }}</p>
          </div>
        ...
        <!-- Last Check / Error -->
        <div v-if="prov.last_check" class="text-xs text-gray-400 mb-3">Last check: {{ prov.last_check }}</div>
        <div v-if="prov.last_error" class="text-xs text-red-500 mb-3 truncate">{{ prov.last_error }}</div>
    ...
    <EmptyState v-if="!selectedCategory"
      ...
      <template #icon>
        <svg class="w-12 h-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M13 10V3L4 14h7v7l9-11h-7z"/></svg>
      </template>
    </EmptyState>
  </div>
</template>
