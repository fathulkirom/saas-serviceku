<template>
  <AuthenticatedLayout>
    ...
    <div class="flex flex-col h-[calc(100vh-64px)] overflow-hidden bg-zinc-100">
      
      <!-- POS Header -->
      <div class="flex items-center justify-between px-6 py-4 bg-white border-b border-zinc-200">
        <div>
          <h1 class="text-xl font-bold text-zinc-900 tracking-tight">Point of Sale (POS)</h1>
          <p class="text-sm text-zinc-500">Kasir Penjualan Langsung</p>
        </div>
        <div class="flex items-center gap-3">
          <Link
            :href="route('keuangan.index')"
            class="px-4 py-2 text-sm font-semibold text-zinc-600 bg-white border border-zinc-200 rounded-xl hover:bg-zinc-50 transition-colors"
          >
            Kembali ke Keuangan
          </Link>
        </div>
      ...
      <div class="flex flex-1 overflow-hidden">
        ...
        <div class="flex-1 flex flex-col min-w-0 bg-white">
          ...
          <div class="flex-1 overflow-y-auto p-6 pt-0">
            <div v-if="filteredProducts.length === 0" class="flex flex-col items-center justify-center h-64 text-zinc-400 bg-zinc-50/50 rounded-3xl border border-zinc-100 border-dashed">
            ...
            <div v-else class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
              <div
                ...
                class="group relative flex flex-col bg-white border border-zinc-200 rounded-2xl p-4 cursor-pointer hover:border-indigo-400 hover:shadow-xl hover:shadow-indigo-500/10 transition-all duration-300 transform hover:-translate-y-1"
              >
                <div class="flex-1">
                  <div class="flex justify-between items-start mb-3">
                    <span class="inline-flex items-center rounded-lg bg-indigo-50 px-2.5 py-1 text-[10px] font-bold text-indigo-700 ring-1 ring-inset ring-indigo-600/20 uppercase tracking-wider">
                      Stok: {{ product.stock_quantity }}
                    </span>
                  </div>
                  <h3 class="text-sm font-bold text-zinc-900 leading-snug mb-1.5 group-hover:text-indigo-600 transition-colors line-clamp-2">
                    {{ product.name }}
                  </h3>
                  <p class="text-xs font-medium text-zinc-500 line-clamp-1" v-if="product.category">{{ product.category }}</p>
                </div>
                <div class="mt-4 pt-4 border-t border-zinc-100 flex items-center justify-between">
                  <span class="text-sm font-black text-zinc-900 tracking-tight">Rp {{ formatNumber(product.selling_price || product.price || 0) }}</span>
                  <div class="w-8 h-8 rounded-full bg-zinc-100 flex items-center justify-center text-zinc-400 group-hover:bg-indigo-600 group-hover:text-white group-hover:rotate-90 transition-all duration-300">
        ...
        <div class="w-[420px] flex flex-col bg-zinc-50 border-l border-zinc-200 z-10 shadow-2xl relative">
          <!-- Customer Selection -->
          <div class="p-5 border-b border-zinc-200 bg-white shadow-sm z-10">
            ...
            <div class="relative group">
              ...
              <div class="absolute inset-y-0 right-0 pr-4 flex items-center pointer-events-none text-zinc-400">
          ...
          <!-- Cart Items -->
          <div class="flex-1 overflow-y-auto p-5 space-y-3 bg-zinc-50/50">
            <div v-if="cart.length === 0" class="flex flex-col items-center justify-center h-full text-zinc-400 text-center">
              <div class="w-20 h-20 bg-zinc-100 rounded-full flex items-center justify-center mb-4">
                <ShoppingCartIcon class="w-10 h-10 opacity-30 text-zinc-600" />
              </div>
              <p class="text-sm font-bold text-zinc-500">Keranjang masih kosong</p>
            ...
            <div
              ...
              :key="index"
              class="flex bg-white border border-zinc-200 rounded-2xl p-4 shadow-sm hover:shadow-md transition-shadow group"
            >
              <div class="flex-1 min-w-0">
                <h4 class="text-sm font-bold text-zinc-900 truncate">{{ item.name }}</h4>
                <div class="text-xs font-semibold text-zinc-500 mt-1">Rp {{ formatNumber(item.price) }}</div>
              </div>
          ...
          <!-- Checkout Summary -->
          <div class="p-6 bg-white border-t border-zinc-200 shadow-[0_-4px_24px_rgba(0,0,0,0.02)]">
            <div class="space-y-3 mb-6">
              <div class="flex justify-between items-center text-sm">
                <span class="text-zinc-500 font-semibold">Subtotal</span>
                <span class="font-bold text-zinc-900">Rp {{ formatNumber(subtotal) }}</span>
              </div>
              <div class="flex justify-between items-center text-sm">
              ...
              <div class="pt-4 border-t border-zinc-100 flex justify-between items-center">
                ...
                <span class="text-2xl font-black text-indigo-600 tracking-tight">Rp {{ formatNumber(total) }}</span>
              </div>
            </div>
