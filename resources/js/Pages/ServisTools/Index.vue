  ...
  <AuthenticatedLayout>
    <template #header>
      <PageHeader :title="pageTitle" :subtitle="subtitle">
        <KButton 
          ...
          class="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-xs font-bold text-white transition-all hover:shadow-md cursor-pointer sk-bg-primary">
          + Template Ceklis
        </KButton>
        <KButton 
          ...
          >
            ...
            <template #cell-action="{ row }">
              <div class="flex items-center justify-end gap-1">
                <KButton  @click="openCeklisModal(row)" class="px-2.5 py-1 rounded text-xs font-medium border sk-border sk-text-primary-brand hover:border-indigo-600 hover:sk-bg-primary-soft">Edit</KButton>
                <KButton  @click="deleteTemplate(row)" class="px-2.5 py-1 rounded text-xs font-medium sk-text-danger border sk-border-primary hover:sk-bg-danger-soft">Hapus</KButton>
              </div>
            </template>
          </KTable>
        ...
        <div class="space-y-6">
          ...
          >
            ...
            <template #cell-expertise="{ row }">
              <span class="text-xs font-semibold px-2 py-0.5 rounded sk-bg-hover sk-text-primary">{{ row.expertise ?? '-' }}</span>
            </template>
            ...
            <template #cell-action="{ row }">
              <div class="flex items-center justify-end gap-1">
                <KButton  @click="openPartnerModal(row)" class="px-2.5 py-1 rounded text-xs font-medium border sk-border sk-text-primary-brand hover:border-indigo-600 hover:sk-bg-primary-soft">Edit</KButton>
                <KButton  @click="deletePartner(row)" class="px-2.5 py-1 rounded text-xs font-medium sk-text-danger border sk-border-primary hover:sk-bg-danger-soft">Hapus</KButton>
              </div>
            </template>
          </KTable>
      ...
      <template #pickup>
        <div class="space-y-6">
          ...
          >
            ...
            <template #cell-action="{ row }">
              <div class="flex items-center justify-end gap-1">
                <KSelect  @change="updatePickupStatus(row, $event.target.value)" class="text-xs py-1 px-2 rounded border font-semibold sk-bg-card sk-border focus:border-indigo-500 focus:ring-indigo-200">
        ...
        <div class="space-y-6">
          ...
          >
            <template #cell-service_info="{ row }">
              ...
              <p class="text-xs sk-text-muted">{{ row.tipe_unit ?? '' }}</p>
            </template>
          ...
          <template v-else>
            <KCard title="Filter Inden">
              <div class="flex flex-wrap items-center gap-3">
                <KSelect  v-model="indentFilters.status" @change="applyIndentFilter" class="text-xs font-semibold rounded-xl border px-3 py-2 sk-bg-card sk-text-primary focus:ring-2 focus:ring-indigo-200 focus:border-indigo-500 focus:outline-none sk-border focus:border-indigo-500 focus:ring-indigo-200">
                ...
                <div class="relative flex-1 min-w-[180px]">
                  <KInput 
                    ...
                    class="w-full rounded-xl border text-xs px-3 py-2 pl-8 focus:ring-2 focus:outline-none sk-bg-card sk-text-primary focus:border-indigo-500 focus:ring-indigo-200 transition-all sk-border focus:border-indigo-500 focus:ring-indigo-200" />
                  <svg class="absolute left-2.5 top-2.5 w-3.5 h-3.5 sk-text-muted" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
                </div>
                <KButton  @click="resetIndentFilter" class="px-3 py-2 rounded-xl text-xs font-semibold border sk-border sk-text-muted hover:sk-bg-hover transition-all sk-bg-card">
                  ↺ Reset
                </KButton>
              </div>
            </KCard>
            ...
            >
              ...
              <template #cell-product_name="{ row }">
                <span class="font-medium sk-text-primary-brand">{{ row.product_name ?? row.product?.name ?? '-' }}</span>
              </template>
              ...
              <template #cell-action="{ row }">
                <div class="flex items-center justify-end gap-1">
                  <a :href="route('indents.print', row.id)" target="_blank" class="px-2.5 py-1 rounded text-xs font-semibold sk-text-primary-brand border border-blue-200 hover:sk-bg-info-soft">Nota</a>
                  <KButton  @click="deleteInden(row)" class="px-2.5 py-1 rounded text-xs font-medium sk-text-danger border sk-border-primary hover:sk-bg-danger-soft">Hapus</KButton>
                </div>
              </template>
            </KTable>
    ...
    <Drawer :open="showCeklisDrawer" :title="editingCeklis ? 'Edit Template Ceklis' : 'Tambah Template Ceklis'" @close="showCeklisDrawer = false" width="450px">
      <form @submit.prevent="submitCeklis" class="space-y-4">
        <div class="space-y-1">
          <label class="text-xs font-semibold sk-text-muted">Nama Template *</label>
          <KInput  v-model="ceklisForm.name" required placeholder="e.g. Ceklis Handphone / Laptop" class="input text-sm" />
        </div>
        <div class="space-y-2">
          <label class="text-xs font-semibold flex items-center justify-between sk-text-muted">
            Item Ceklis (Daftar Fisik & Kelengkapan)
            <KButton  type="button" @click="addCeklisItem" class="text-xs sk-text-primary-brand font-bold hover:underline">+ Tambah Item</KButton>
          </label>
          <div v-for="(item, idx) in ceklisForm.items" :key="idx" class="flex gap-2 items-center">
            ...
            <KButton  type="button" @click="ceklisForm.items.splice(idx, 1)" class="sk-text-danger font-bold text-xs px-2 py-1">✕</KButton>
          </div>
        </div>
    ...
    <Drawer :open="showPickupDrawer" title="Jadwal Pickup / Delivery Baru" @close="showPickupDrawer = false" width="450px">
      <form @submit.prevent="submitPickup" class="space-y-4">
        <div class="space-y-1">
          <label class="text-xs font-semibold sk-text-muted">Tipe Layanan *</label>
        ...
        <div class="space-y-1">
          <label class="text-xs font-semibold sk-text-muted">Alamat Penjemputan / Pengantaran *</label>
          <KTextarea  v-model="pickupForm.address" required rows="2" placeholder="Jl. Anggrek No. 123..." class="input text-sm"></KTextarea>
        </div>
        <div class="space-y-1">
          <label class="text-xs font-semibold sk-text-muted">Tanggal & Waktu Penjadwalan *</label>
          <KInput  v-model="pickupForm.scheduled_date" type="datetime-local" required class="input text-sm" />
        </div>
        <div class="space-y-1">
          <label class="text-xs font-semibold sk-text-muted">Catatan / Instruksi Driver</label>
