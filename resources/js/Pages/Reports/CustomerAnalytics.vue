    ...
    <div class="flex flex-col min-h-[calc(100vh-64px)] bg-zinc-50/50">
        <div class="px-6 sm:px-8 py-6 bg-white border-b border-zinc-200 flex flex-col sm:flex-row sm:items-center justify-between gap-4 sticky top-0 z-20">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-2xl font-bold text-zinc-900 tracking-tight">Analisis Pelanggan</h1>
                    <p class="text-sm text-zinc-500 font-medium mt-0.5">Statistik pengguna dan tren kunjungan bengkel</p>
                </div>
            </div>
        </div>
        ...
        <div class="flex-1 p-6 sm:p-8 max-w-[1400px] mx-auto w-full space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="bg-white rounded-2xl border border-zinc-200 shadow-sm p-5 flex items-center gap-4">
                    ...
                    <div>
                        <p class="text-xs font-bold text-zinc-500 uppercase tracking-wider">Total Pelanggan</p>
                        <p class="text-2xl font-black text-zinc-900">{{ totalCustomers }} <span class="text-sm font-bold text-zinc-500">Orang</span></p>
                    </div>
                </div>
                <div class="bg-white rounded-2xl border border-zinc-200 shadow-sm p-5 flex items-center gap-4">
                    ...
                    <div>
                        <p class="text-xs font-bold text-zinc-500 uppercase tracking-wider">Pelanggan Aktif (3 Bln)</p>
                        <p class="text-2xl font-black text-blue-600">{{ activeCustomers }} <span class="text-sm font-bold text-blue-400">Orang</span></p>
                    </div>
                </div>
                <div class="bg-white rounded-2xl border border-zinc-200 shadow-sm p-5 flex items-center gap-4">
                    <div class="w-12 h-12 rounded-xl flex items-center justify-center shadow-inner" 
                        :class="retentionRate > 50 ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/></svg>
                    </div>
                    <div>
                        <p class="text-xs font-bold text-zinc-500 uppercase tracking-wider">Tingkat Retensi</p>
                        <p class="text-2xl font-black" :class="retentionRate > 50 ? 'text-emerald-600' : 'text-amber-600'">{{ retentionRate }}%</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl border border-zinc-200 shadow-sm overflow-hidden flex flex-col">
                <div class="px-6 py-5 border-b border-zinc-200 bg-zinc-50">
                    <h3 class="text-lg font-bold text-zinc-900">Jam Sibuk Bengkel (Peak Hours)</h3>
                    <p class="text-xs text-zinc-500 mt-1 font-medium">Berdasarkan waktu pembuatan nota servis</p>
                </div>
                <div class="p-6 sm:p-8">
                    <div v-if="peakHours.length" class="space-y-4 max-w-2xl">
                        <div v-for="ph in peakHours" :key="ph.hour" class="flex items-center gap-4 group">
                            <span class="text-sm font-bold text-zinc-900 w-16">{{ ph.hour }}:00</span>
                            <div class="flex-1 bg-zinc-100 rounded-full h-4 overflow-hidden shadow-inner">
                            ...
                            <span class="text-sm font-black text-indigo-600 w-24 text-right">{{ ph.total }} <span class="text-xs font-bold text-zinc-400 uppercase">Servis</span></span>
                        </div>
                    </div>
                    <div v-else class="text-center py-12">
                        ...
                        <p class="text-sm font-bold text-zinc-900">Belum ada data jam sibuk</p>
                        <p class="text-xs text-zinc-500 mt-1">Data akan muncul setelah ada transaksi servis.</p>
                    </div>
                </div>
            </div>
        </div>
