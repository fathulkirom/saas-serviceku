        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                    ...
                    <form @submit.prevent="updateSettings">
                        <div class="grid grid-cols-2 gap-4">
                            <div class="col-span-2">
                                <label class="block text-sm font-medium text-gray-700">Lokasi Backup (HDD)</label>
                                <input v-model="form.backup_path" type="text"
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm"
                                    placeholder="/mnt/hdd/Backup/ServiceKU" />
                                <p class="mt-1 text-xs text-gray-400">Path ke direktori HDD untuk menyimpan backup</p>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Simpan Backup (hari)</label>
                                <input v-model.number="form.backup_retention_days" name="backup_retention_days" type="number" min="1" max="365"
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm" />
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Jam Backup Otomatis</label>
                            ...
                            <div class="col-span-2">
                                <label class="relative inline-flex items-center cursor-pointer">
                                    ...
                                    <span class="ml-3 text-sm font-medium text-gray-700">Aktifkan Backup Otomatis (via cron)</span>
                                </label>
                                <p class="mt-1 text-xs text-gray-400">Pastikan cron job sudah diset: <code class="bg-gray-100 px-1 rounded">* * * * * cd {{ basePath }} && php artisan schedule:run &gt;&gt; /dev/null 2&gt;&amp;1</code></p>
                            </div>
                        </div>
                        ...
                        <div class="mt-6 pt-4 border-t">
                            ...
                            <p class="text-xs text-gray-500 mb-3">Backup otomatis diupload ke Google Drive sebagai cadangan off-site. <a href="https://rclone.org/drive/" target="_blank" class="text-indigo-600 hover:underline">Cara setup rclone</a></p>
                            <div class="grid grid-cols-2 gap-4">
                                <div class="col-span-2">
                                    <label class="relative inline-flex items-center cursor-pointer">
                                        <input type="checkbox" v-model="gdriveEnabled" class="sr-only peer" />
                                        <div class="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-indigo-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-indigo-600"></div>
                                        <span class="ml-3 text-sm font-medium text-gray-700">Upload ke Google Drive</span>
                                    </label>
                                </div>
                                <div class="col-span-2">
                                    <label class="block text-sm font-medium text-gray-700">Folder ID Google Drive</label>
                                    <input v-model="form.gdrive_folder_id" type="text" placeholder="xxx_FOLDER_ID_xxx"
                                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm" />
                                    <p class="mt-1 text-xs text-gray-400">Dapatkan dari URL folder Google Drive: <code class="bg-gray-100 px-1 rounded">https://drive.google.com/drive/folders/xxx_FOLDER_ID_xxx</code></p>
                                </div>
                                <div class="col-span-2">
                                    <label class="relative inline-flex items-center cursor-pointer">
                                        <input type="checkbox" v-model="gdriveDeleteLocal" class="sr-only peer" />
                                        <div class="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-indigo-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-indigo-600"></div>
                                        <span class="ml-3 text-sm font-medium text-gray-700">Hapus file lokal setelah upload (hemat SSD)</span>
                                    </label>
                                </div>
                            </div>
                        </div>
                ...
                <div class="rounded-2xl p-6 border" style="background: var(--bg-card); border-color: var(--border-color);">
                    ...
                    <div v-if="backupFiles.length === 0" class="text-center py-8 text-gray-400">
                        ...
                        <div v-for="file in backupFiles" :key="file.path"
                            ...
                            <div class="flex items-center gap-3 min-w-0">
                                ...
                                <div class="min-w-0">
                                    ...
                                    <p class="text-xs text-gray-500">{{ file.date }} • {{ file.size }}</p>
                                </div>
                            </div>
            ...
            <div class="space-y-6">
                ...
                <div class="rounded-2xl p-6 border" style="background: var(--bg-card); border-color: var(--border-color);">
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="flex justify-between text-sm">
                            ...
                            <span :class="config.backup_last_status === 'success' ? 'text-green-600' : 'text-red-600'"
                                class="font-medium">
                        ...
                        <div class="flex justify-between text-sm">
                            <span class="text-gray-500">Total File:</span>
                ...
                <div class="rounded-2xl p-6 border" style="background: var(--bg-card); border-color: var(--border-color);">
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="mt-2">
                            ...
                            <p class="text-xs text-gray-400 mt-1">{{ diskInfo.percent }}% terpakai</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="rounded-2xl p-6 border" style="background: var(--bg-card); border-color: var(--border-color);">
                    ...
                    <div v-if="gdriveInfo" class="space-y-3">
                        <div class="flex justify-between text-sm">
                            <span class="text-gray-500">rclone:</span>
                            <span :class="gdriveInfo.rclone_installed ? 'text-green-600' : 'text-red-600'" class="font-medium">
                                {{ gdriveInfo.rclone_installed ? '✅ ' + (gdriveInfo.rclone_version || '') : '❌ Tidak terinstall' }}
                            </span>
                        </div>
                        <div class="flex justify-between text-sm">
                            <span class="text-gray-500">Remote:</span>
                            <span :class="gdriveInfo.remote_configured ? 'text-green-600' : 'text-yellow-600'" class="font-medium">
                                {{ gdriveInfo.remote_configured ? '✅ Terkonfigurasi' : '❌ Belum' }}
                            </span>
                        </div>
                        <div class="flex justify-between text-sm">
                            <span class="text-gray-500">Upload otomatis:</span>
                        ...
                        <div v-if="gdriveFiles.length > 0" class="flex justify-between text-sm">
                            <span class="text-gray-500">File di Drive:</span>
                ...
                <div class="rounded-2xl p-6 border" style="background: var(--bg-card); border-color: var(--border-color);">
                    ...
                    <ol class="text-sm text-gray-600 space-y-1.5 list-decimal list-inside">
                        <li>Install rclone: <code class="bg-gray-100 px-1 rounded text-xs">brew install rclone</code></li>
                        <li>Konfigurasi: <code class="bg-gray-100 px-1 rounded text-xs">rclone config</code></li>
                    ...
                    <p class="text-xs text-gray-400 mt-2">Script setup: <code class="bg-gray-100 px-1 rounded">docker/cloudflare/setup-gdrive.sh</code></p>
                </div>

                <!-- Panduan Cron -->
                <div class="rounded-2xl p-6 border" style="background: var(--bg-card); border-color: var(--border-color);">
                    <h3 class="text-lg font-semibold text-gray-900 mb-2">🤖 Cron Job</h3>
                    <p class="text-sm text-gray-500 mb-3">Untuk backup otomatis, tambahkan ke crontab:</p>
                    <div class="bg-gray-800 text-green-400 rounded-md p-3 text-xs font-mono">
                        # Setiap jam 03:00 pagi<br>
                        <span class="text-yellow-300">0 3</span> * * * cd {{ basePath }} && php artisan backup:run --force
                    </div>
                    <p class="text-xs text-gray-400 mt-2">Atau biarkan Laravel Scheduler menanganinya:</p>
                    <div class="bg-gray-800 text-green-400 rounded-md p-3 text-xs font-mono mt-2">
