<template>
    <AdminLayout>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <div v-for="plan in plans" :key="plan.id" class="rounded-2xl p-6 border transition-all duration-300 hover:-translate-y-0.5"
                ...
                <div class="flex justify-between items-start">
                    ...
                    <span :class="plan.is_active ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-red-500/10 text-red-400 border border-red-500/20'"
                ...
                <div class="mt-2 flex items-baseline gap-2">
                    <template v-if="plan.is_promo_active">
                        <p class="text-xl text-gray-400 line-through">Rp {{ formatNumber(plan.price) }}</p>
                ...
                <div class="p-6">
                    ...
                    <form @submit.prevent="submitForm">
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div class="col-span-2 sm:col-span-1">
                                <label class="block text-sm font-medium text-gray-700">Harga Normal (Rp)</label>
                            ...
                            <div class="col-span-2 sm:col-span-1">
                                <label class="block text-sm font-medium text-gray-700">Trial (hari)</label>
                            ...
                            <div class="col-span-2 border-t pt-3 mt-1">
                                <div class="flex items-center gap-2 mb-3">
                                    <h4 class="text-sm font-semibold text-gray-800">🎉 Promo Diskon</h4>
                                    <span class="text-[10px] text-gray-400">(opsional)</span>
                                </div>
                                <div class="grid grid-cols-2 gap-4">
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700">Harga Promo (Rp)</label>
                                    ...
                                    <div></div>
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700">Mulai Promo</label>
                                        <input v-model="form.promo_start" type="date"
                                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500 text-sm">
                                    </div>
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700">Selesai Promo</label>
                        ...
                        <div class="mt-6 border-t pt-4">
                            <div class="mb-4">
                                <label class="block text-sm font-semibold text-gray-800 mb-1">Akses Fitur per Tipe Bisnis</label>
                                <p class="text-xs text-gray-400 mb-2">Pilih tipe bisnis di bawah ini untuk mengatur hak akses fiturnya secara khusus.</p>
                                
                                <select v-model="activeTabBusinessType" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm bg-indigo-50/50">
                            ...
                            <div class="bg-gray-50 p-4 rounded-xl border border-gray-200 mb-4">
                                ...
                                <div class="max-h-60 overflow-y-auto pr-1 space-y-2">
                                    <div v-for="(feature, key) in KNOWN_FEATURES" :key="key" class="flex items-center gap-2">
                                        <span class="w-1/2 text-xs font-medium text-gray-700">{{ feature }}</span>
                                        <select v-model="featureLevels[activeTabBusinessType][key]"
                                            class="w-1/2 py-1 rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-xs">
                            ...
                            <div class="mt-4 pt-3 border-t">
                                <label class="text-sm font-medium text-gray-700 mb-2 block">Batas Numerik Paket</label>
                                <div class="grid grid-cols-2 gap-3">
                                    <div>
                                        <label class="block text-xs text-gray-500">Max User</label>
                                        <input v-model.number="numericLimits.max_users" name="max_users" type="number" min="0"
                                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                                    </div>
                                    <div>
                                        <label class="block text-xs text-gray-500">Max Cabang</label>
                        ...
                        <!-- Business Types -->
                        <div class="mt-6">
                            <label class="text-sm font-medium text-gray-700 mb-2 block">Tipe Bisnis yang Didukung</label>
                            <p class="text-xs text-gray-400 mb-2">Pilih tipe bisnis yang diizinkan untuk paket ini. Kosongkan = semua tipe didukung.</p>
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                <label v-for="(label, key) in BUSINESS_TYPES" :key="key"
                                    class="flex items-start gap-2 p-2 rounded border cursor-pointer hover:bg-gray-50"
                                    :class="selectedBusinessTypes.includes(key) ? 'border-indigo-300 bg-indigo-50' : 'border-gray-200'">
