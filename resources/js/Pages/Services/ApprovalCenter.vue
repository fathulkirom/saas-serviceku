    ...
    <AuthenticatedLayout>
        <div class="flex flex-col gap-6">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h2 class="text-2xl font-bold tracking-tight text-zinc-900">Pusat Persetujuan</h2>
                    <p class="text-sm text-zinc-500 mt-1">Semua item yang menunggu approval owner/admin.</p>
                </div>
            </div>
            ...
            <ApprovalSection title="Quotation Menunggu Persetujuan" :count="quotations?.length" :empty="'Tidak ada quotation menunggu.'">
                <div class="divide-y divide-zinc-100">
                    <div v-for="q in quotations ?? []" :key="q.id" class="flex items-center justify-between gap-4 px-6 py-3">
                        <div>
                            <p class="text-sm font-semibold text-zinc-900">#{{ q.service_id }} — {{ q.service?.customer?.name }}</p>
                            <p class="text-xs text-zinc-500">{{ q.total_amount ? 'Rp ' + formatNumber(q.total_amount) : '' }}</p>
                        </div>
                        <Link :href="route('services.show', q.service_id)" class="text-xs font-bold px-3 py-1.5 rounded-lg text-indigo-600 bg-indigo-50 hover:bg-indigo-100">Detail</Link>
                    </div>
                </div>
            </ApprovalSection>
            ...
            <ApprovalSection title="Perubahan Harga" :count="priceChanges?.length" :empty="'Tidak ada perubahan harga.'">
                <div class="divide-y divide-zinc-100">
                    <div v-for="p in priceChanges ?? []" :key="p.id" class="flex items-center justify-between gap-4 px-6 py-3">
                        <div>
                            <p class="text-sm font-semibold text-zinc-900">#{{ p.service_id }} — {{ p.service?.customer?.name }}</p>
                            <p class="text-xs text-zinc-500">{{ p.requested_by || '—' }}</p>
                        </div>
            ...
            <ApprovalSection title="Permintaan Reopen" :count="reopens?.length" :empty="'Tidak ada permintaan reopen.'">
                <div class="divide-y divide-zinc-100">
                    <div v-for="r in reopens ?? []" :key="r.id" class="flex items-center justify-between gap-4 px-6 py-3">
                        <div>
                            <p class="text-sm font-semibold text-zinc-900">#{{ r.service_id }} — {{ r.service?.customer?.name }}</p>
                            <p class="text-xs text-zinc-500">{{ r.reason || '—' }}</p>
                        </div>
                        <div class="flex items-center gap-2">
                            <Link :href="route('services.show', r.service_id)" class="text-xs font-bold px-3 py-1.5 rounded-lg text-indigo-600 bg-indigo-50 hover:bg-indigo-100">Detail</Link>
                            <button type="button" :disabled="approving === r.id" @click="approveReopen(r.id)"
                                class="text-xs font-bold px-3 py-1.5 rounded-lg text-emerald-700 bg-emerald-50 hover:bg-emerald-100 disabled:opacity-50">
            ...
            <ApprovalSection title="Retur Penjualan" :count="returns?.length" :empty="'Tidak ada retur.'">
                <div class="divide-y divide-zinc-100">
                    <div v-for="r in returns ?? []" :key="r.id" class="flex items-center justify-between gap-4 px-6 py-3">
                        <div>
                            <p class="text-sm font-semibold text-zinc-900">Retur #{{ r.id }} — {{ r.sale?.customer?.name }}</p>
                            <p class="text-xs text-zinc-500">{{ r.reason || '—' }}</p>
                        </div>
                        <span class="text-xs font-bold px-3 py-1.5 rounded-lg text-amber-700 bg-amber-50">Pending</span>
                    </div>
                </div>
            </ApprovalSection>
            ...
            <ApprovalSection title="Penyesuaian Stok" :count="stockAdjustments?.length" :empty="'Tidak ada penyesuaian.'">
                <div class="divide-y divide-zinc-100">
                    <div v-for="s in stockAdjustments ?? []" :key="s.id" class="flex items-center justify-between gap-4 px-6 py-3">
                        <div>
                            <p class="text-sm font-semibold text-zinc-900">{{ s.product?.name || '—' }}</p>
                            <p class="text-xs text-zinc-500">{{ s.reason || '—' }}</p>
                        </div>
                        <span class="text-xs font-bold px-3 py-1.5 rounded-lg text-amber-700 bg-amber-50">Pending</span>
                    </div>
                </div>
