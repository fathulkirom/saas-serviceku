        ...
        <div class="flex flex-col gap-6">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-2">
                ...
                <div class="flex items-center gap-2">
                    <button @click="clearFilters" class="hidden sm:inline-flex items-center gap-2 px-4 py-2 bg-white hover:bg-zinc-50 border border-zinc-200 text-zinc-700 text-sm font-bold rounded-xl transition-colors shadow-sm">
                ...
                <div class="flex flex-col md:flex-row md:items-end gap-4">
                    <div class="flex-1 min-w-[200px]">
                        ...
                        <div class="relative">
                            ...
                            <input type="text" v-model="filters.customer_name" placeholder="Nama pelanggan..." class="w-full rounded-xl border border-zinc-300 text-sm pl-10 pr-4 py-2.5 focus:ring-2 focus:ring-indigo-100 focus:border-indigo-500 transition-all text-zinc-900 bg-white" />
                        </div>
                    </div>
                    <div class="flex-1 min-w-[150px]">
                        <label class="block text-xs font-bold text-zinc-500 uppercase tracking-wider mb-2">No. Telepon / WA</label>
                        <input type="text" v-model="filters.phone" placeholder="0812..." class="w-full rounded-xl border border-zinc-300 text-sm px-4 py-2.5 focus:ring-2 focus:ring-indigo-100 focus:border-indigo-500 transition-all text-zinc-900 bg-white" />
                    </div>
                    <div class="flex-1 min-w-[150px]">
                        <label class="block text-xs font-bold text-zinc-500 uppercase tracking-wider mb-2">Nomor Tiket (SR)</label>
                        <input type="text" v-model="filters.sr_code" placeholder="SR1000..." class="w-full rounded-xl border border-zinc-300 text-sm px-4 py-2.5 focus:ring-2 focus:ring-indigo-100 focus:border-indigo-500 transition-all text-zinc-900 bg-white" />
                    </div>
                    <div class="flex-1 min-w-[200px]">
                        <label class="block text-xs font-bold text-zinc-500 uppercase tracking-wider mb-2">Status Servis</label>
                        <select v-model="activeFilter" class="w-full rounded-xl border border-zinc-300 text-sm px-4 py-2.5 focus:ring-2 focus:ring-indigo-100 focus:border-indigo-500 transition-all text-zinc-900 bg-white">
            ...
            <div v-if="selectedIds.length > 0" class="flex items-center justify-between p-4 rounded-2xl bg-indigo-50 text-indigo-900 border border-indigo-200 shadow-sm animate-in slide-in-from-top-2">
                ...
                <div class="flex items-center gap-2">
                    <button v-if="selectedAcceptableIds.length > 0" @click="bulkUpdateStatus('diterima')" class="px-4 py-2 text-sm font-bold bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl transition-colors shadow-sm">
                        Terima Tiket ({{ selectedAcceptableIds.length }})
                    </button>
                    <button v-if="selectedCancelableIds.length > 0" @click="bulkUpdateStatus('cancel')" class="px-4 py-2 text-sm font-bold bg-red-600 hover:bg-red-700 text-white rounded-xl transition-colors shadow-sm">
                        Batalkan ({{ selectedCancelableIds.length }})
                    </button>
                    <button @click="selectedIds = []" class="px-4 py-2 text-sm font-bold text-indigo-700 bg-white border border-indigo-200 hover:bg-indigo-50 rounded-xl transition-colors shadow-sm">
                        Batal Pilihan
                    </button>
                </div>
            </div>
                        ...
                        <thead class="text-xs font-bold text-zinc-500 uppercase bg-zinc-50/50 border-b border-zinc-200 tracking-wider">
                            <tr>
                                <th scope="col" class="px-5 py-4 w-10 text-center">
                                    <input type="checkbox" :checked="allSelected" @change="toggleAll" class="w-4 h-4 rounded border-zinc-300 text-indigo-600 focus:ring-indigo-600 cursor-pointer" :disabled="selectableServiceIds.length === 0" />
                                </th>
                        ...
                        <tbody class="divide-y divide-zinc-100">
                            ...
                            <tr v-for="service in paginatedServices" :key="service.id" 
                                ...
                                <td class="px-5 py-4 text-center" @click.stop>
                                    <input type="checkbox" :value="service.id" v-model="selectedIds" class="w-4 h-4 rounded border-zinc-300 text-indigo-600 focus:ring-indigo-600 cursor-pointer" :disabled="!canBulkActOnService(service)" />
                                </td>
                                ...
                                <td class="px-5 py-4 text-right">
                                    <div class="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <button @click.stop="router.visit(route('services.show', service.id))" class="w-8 h-8 rounded-lg flex items-center justify-center text-zinc-400 hover:text-indigo-600 hover:bg-indigo-50 transition-colors" title="Detail">
                                        ...
                                        <button v-if="canCancel(service)" @click.stop="confirmCancel(service)" class="w-8 h-8 rounded-lg flex items-center justify-center text-zinc-400 hover:text-red-600 hover:bg-red-50 transition-colors" title="Batalkan">
            ...
            <div v-if="showCancelModal" class="fixed inset-0 z-50 flex items-center justify-center bg-zinc-900/40 backdrop-blur-sm" @click.self="showCancelModal = false">
                <div class="bg-white w-full max-w-sm mx-4 rounded-2xl shadow-xl border border-zinc-200 overflow-hidden animate-in zoom-in-95 duration-200">
                    ...
                    <div class="p-6 pt-0 flex gap-3">
                        <button class="flex-1 px-4 py-2.5 rounded-xl text-sm font-bold text-zinc-700 bg-white border border-zinc-300 hover:bg-zinc-50 transition-colors" @click="showCancelModal = false">Batal</button>
                        <button class="flex-1 px-4 py-2.5 rounded-xl text-sm font-bold text-white bg-red-600 hover:bg-red-700 transition-colors flex justify-center items-center gap-2" @click="executeCancel" :disabled="processingAction === 'cancel'">
                ...
                <div class="bg-white w-full max-w-sm mx-4 rounded-2xl shadow-xl border border-zinc-200 overflow-hidden animate-in zoom-in-95 duration-200">
                    <div class="p-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <select v-model="selectedTechnicianId" class="w-full rounded-xl border border-zinc-300 text-sm px-4 py-2.5 focus:ring-2 focus:ring-indigo-100 focus:border-indigo-500 transition-all text-zinc-900 bg-white">
                    ...
                    <div class="p-6 pt-0 flex gap-3">
                        <button class="flex-1 px-4 py-2.5 rounded-xl text-sm font-bold text-zinc-700 bg-white border border-zinc-300 hover:bg-zinc-50 transition-colors" @click="showAlokasiModal = false">Batal</button>
                        <button class="flex-1 px-4 py-2.5 rounded-xl text-sm font-bold text-white bg-indigo-600 hover:bg-indigo-700 transition-colors disabled:opacity-50 flex justify-center items-center gap-2" @click="executeAlokasi" :disabled="!selectedTechnicianId || processingAction === 'alokasi'">
