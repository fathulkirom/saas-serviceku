<template>
    ...
    <GuestLayout>
        <div class="text-center mb-8">
            ...
            <h1 class="text-2xl font-black sk-text-primary tracking-tight mb-2">Verifikasi Dua Langkah</h1>
            <p class="sk-text-muted font-medium text-sm leading-relaxed">
                Masukkan kode keamanan untuk <strong class="sk-text-primary font-bold">{{ email }}</strong>.
            </p>
        </div>
        ...
        <div v-if="form.errors.code" class="mb-6 p-4 sk-bg-danger-soft border border-red-100 rounded-xl flex items-start gap-3">
            <svg class="w-5 h-5 sk-text-danger shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <p class="text-sm font-semibold sk-text-danger">{{ form.errors.code }}</p>
        </div>

        <!-- TOTP Form -->
        <form v-if="!showBackup && !showEmailCode" @submit.prevent="submit" class="space-y-6">
            <div>
                <label class="block text-sm font-bold sk-text-primary mb-2 text-center">Kode Aplikasi Authenticator</label>
                <KInput 
                    v-model="form.code"
                    type="text"
                    inputmode="numeric"
                    maxlength="6"
                    placeholder="000000"
                    class="w-full text-center text-3xl font-black tracking-[0.5em] px-4 py-4 rounded-xl border sk-border sk-bg-card sk-text-primary focus:ring-2 focus:ring-indigo-100 focus:border-indigo-500 outline-none transition-all shadow-sm"
                    autofocus />
            </div>
        ...
        <form v-if="showBackup" @submit.prevent="submitBackup" class="space-y-6">
            <div class="sk-bg-warning-soft text-amber-900 border border-amber-100 p-4 rounded-xl mb-2 text-sm font-medium">
                Masukkan salah satu dari kode pemulihan cadangan Anda.
            </div>
            <div>
                <label class="block text-sm font-bold sk-text-primary mb-2 text-center">Kode Cadangan</label>
                <KInput 
                    v-model="form.code"
                    type="text"
                    placeholder="XXXXXX-XXXXXX"
                    class="w-full text-center text-xl font-bold tracking-widest px-4 py-4 rounded-xl border sk-border sk-bg-card sk-text-primary focus:ring-2 focus:ring-indigo-100 focus:border-indigo-500 outline-none transition-all shadow-sm"
                    autofocus />
            </div>
            ...
            <div class="pt-2 flex flex-col gap-4">
                ...
                <KButton  type="button" @click="showBackup = false; form.code = ''"
                    class="w-full py-2 text-sm font-bold sk-text-muted hover:sk-text-primary transition-colors">
        ...
        <form v-if="showEmailCode" @submit.prevent="submitEmailCode" class="space-y-6">
            <div class="sk-bg-primary-soft text-indigo-900 border sk-border-primary p-4 rounded-xl mb-2 text-sm font-medium">
                Kode keamanan telah dikirim ke email <strong class="font-bold">{{ email }}</strong>. Silakan periksa kotak masuk Anda.
            </div>
            <div>
                <label class="block text-sm font-bold sk-text-primary mb-2 text-center">Kode dari Email</label>
                <KInput 
                    v-model="form.code"
                    type="text"
                    inputmode="numeric"
        ...
        <!-- Options -->
        <div v-if="!showBackup && !showEmailCode" class="mt-8 space-y-3 pt-6 border-t sk-border-light">
            <KButton  @click="sendEmailCode" class="w-full flex items-center justify-center gap-2 py-2 text-sm font-bold sk-text-primary-brand hover:sk-text-primary-brand transition-colors">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>
                Kirim kode keamanan via Email
            </KButton>
            <KButton  @click="showBackup = true" class="w-full flex items-center justify-center gap-2 py-2 text-sm font-bold sk-text-muted hover:sk-text-primary transition-colors">
