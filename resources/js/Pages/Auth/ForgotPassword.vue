<template>
    <GuestLayout>
        <div class="text-center mb-8">
            <h2 class="text-xl font-bold sk-text-primary mb-2">Lupa Password?</h2>
            <p class="text-sm sk-text-muted">Masukkan email Anda dan kami akan mengirimkan link reset password.</p>
        </div>
        ...
        <!-- Error Message -->
        <div v-if="form.errors.email" class="mb-6 p-4 sk-bg-danger-soft border border-red-100 rounded-xl flex items-start gap-3">
            <svg class="w-5 h-5 sk-text-danger shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <p class="text-sm font-semibold sk-text-danger">{{ form.errors.email }}</p>
        </div>

        <form @submit.prevent="submit" class="space-y-6">
            <div>
                <label for="email" class="block text-sm font-bold sk-text-primary mb-2">Email Akun</label>
                <KInput 
                    id="email"
                    v-model="form.email"
                    type="email"
