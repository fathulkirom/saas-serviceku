<template>
    <GuestLayout>
        <div class="text-center mb-8">
            <h2 class="text-xl font-bold text-zinc-900 mb-2">Masuk ke <span class="text-indigo-600">{{ storeName }}</span></h2>
            <p class="text-sm text-zinc-500">Masukkan email dan password untuk masuk ke dasbor</p>
        </div>
        ...
        <!-- Error Message -->
        <div v-if="form.errors.login || form.errors.password" class="mb-6 p-4 bg-red-50 border border-red-100 rounded-xl flex items-start gap-3">
            <svg class="w-5 h-5 text-red-600 shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <div class="flex-1">
                <p v-if="form.errors.login" class="text-sm font-semibold text-red-800">{{ form.errors.login }}</p>
                <p v-else-if="form.errors.password" class="text-sm font-semibold text-red-800">{{ form.errors.password }}</p>
            </div>
        </div>
        ...
        <form @submit.prevent="submit" class="space-y-5">
            ...
            <div>
                <div class="flex items-center justify-between mb-2">
                    <label for="password" class="block text-sm font-bold text-zinc-900">Password</label>
        ...
        <div class="mt-8">
            <div class="relative">
                ...
                <div class="relative flex justify-center text-sm">
                    <span class="px-3 bg-white text-zinc-400 font-medium">atau masuk dengan</span>
                </div>
            </div>
            
            <div class="mt-6">
                <a :href="route('google.login')"
                    class="w-full flex items-center justify-center gap-3 px-4 py-3 border border-zinc-200 rounded-xl bg-white hover:bg-zinc-50 hover:shadow-sm transition-all text-sm font-bold text-zinc-700 shadow-sm">
...
const demoAccounts = [
    { role: 'owner', label: 'Owner', email: 'demo@serviceku.app', icon: '👑', bg: 'bg-indigo-600', url: '/dev-login?role=owner' },
